...
class PaymentSerializer(serializers.ModelSerializer):
    ...
    def get_student_financials(self, student):
        ...
        if student.current_level:
            fee_struct = FeeStructure.objects.filter(level=student.current_level).first()
            if fee_struct:
                total_fees = float(fee_struct.total_fee)
                
        total_paid = float(sum(
            p.amount for p in student.payments.filter(status=Payment.Status.ALLOCATED)
        ))
...
class BulkAllocationSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        total_allocated = sum(item['amount'] for item in allocations_data)
